    ...
    void Update()
    {
        ...
        switch (state)
        {
            case AIState.WANDERING:
                knightControl.SetTarget(allWaypoints[currentWaypoint].transform);
                //Debug.Log(currentWaypoint);
                if ((Vector3.Distance(knightControl.target.transform.position, transform.position) < 2.0f))
                {
            ...
            case AIState.CHASING:
                knightControl.SetTarget(GameObject.FindGameObjectWithTag("Player").transform);
                // UNCOMMENT THIS ONE //animator.SetBool("IsWalking", true);
                if (!CanSeePlayer())
                {
                    //i can't see him, so back to wandering...
                    state = AIState.WANDERING;
                    //Debug.Log(this.name + " WANDERING");
                    // UNCOMMENT THIS ONE //animator.SetBool("IsWalking", false);
                    return;
                }
                if (Vector3.Distance(knightControl.target.transform.position, transform.position) <= GetComponent<SphereCollider>().radius)
                {
            ...
            case AIState.ATTACKING:
                if (Vector3.Distance(knightControl.target.transform.position, transform.position) <= GetComponent<SphereCollider>().radius)
                {
                    // UNCOMMENT THIS ONE //animator.SetBool("IsAttacking", true);
                    //Debug.Log(this.name + " I AM ATTACKING");
                }
                else
                {
                    state = AIState.CHASING;
                    // UNCOMMENT THIS ONE //animator.SetBool("IsAttacking", false);
                    //Debug.Log(this.name + " CHASING");
                }
                break;
            case AIState.DEAD:
